...
# 需定义环境变量 RUN_ENV：取值范围：local：本机；dev：开发环境；sit：测试环境：uat：预发布环境；prod：生产环境
...
PID_PATH="/data/webroot/${APP_NAME}"
...
then
    Xmx="512m"
elif [ "${SERVER_ENVIROMENT}"x = "sit"x ]
then
    Xmx="512m"
elif [ "${SERVER_ENVIROMENT}"x = "uat"x ]
then
    Xmx="256m"
elif [ "${SERVER_ENVIROMENT}"x = "prod"x ]
then
    Xmx="4096m"
else
    echo "Environment variables RUN_ENV not found. 需定义环境变量 RUN_ENV：取值范围：local：本机；dev：开发环境；sit：测试环境：uat：预发布环境；prod：生产环境"
    exit -1
fi
